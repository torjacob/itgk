...
def number_frequency(filename):
    with open(filename) as f:
        ...
        content = [int(i) for i in content] # Omformer innhold til en liste
        num_dict = {}
        
        
        for i in range(len(content)):
            num = content[i]
            counting = content.count(content[i])
            num_dict.update({num: counting})
...
print_dict(number_frequency('numbers.txt'))
